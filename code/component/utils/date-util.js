...
export const getI18nSettings = () => {
  ...
  return {
    dayNamesShort: weeks.map(week => iweek[week]),
    dayNames: weeks.map(week => iweek[week]),
    monthNamesShort: months.map(month => imonth[month]),
    monthNames: months.map((month, index) => imouths[month+(index + 1)]),
    amPm: ['am', 'pm']
  };
};
